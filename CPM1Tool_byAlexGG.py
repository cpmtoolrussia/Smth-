...
SAVE_URL = "https://europe-west1-cp-multiplayer.cloudfunctions.net/SavePlayerRecordsPartially7"
RANK_URL = "https://us-central1-cp-multiplayer.cloudfunctions.net/SetUserRating5"

MAX_MONEY = 50_000_000
MAX_COIN  = 500_000

CLONE_DOMAIN        = "alexgg.cpm"
CLONE_MAX_DEFAULT   = 20
CLONE_MAX_OWNER     = 500
CLONE_COOLDOWN_SEC  = 600
CLONE_DELAY_SEC     = 2.0
CLONE_FRIENDS_CLEAR = True
CLONE_NAME_SUFFIX   = True
...
FIELD_MAPPING = [
    ...
    (44,"carIDnStatus"),(43,"platesData"),(45,"flags"),
...
class CPMNuker:
    ...
    async def login(self, email, password):
        ...
        h = {"Accept":"*/*","Accept-Encoding":"gzip","Content-Type":"application/json",
             "User-Agent":"UnityPlayer/2022.3.62f2 (UnityWebRequest/1.0, libcurl/8.10.1-DEV)",
             "X-Unity-Version":"2022.3.62f2"}
        p = {"email":email,"password":password,"returnSecureToken":True,
        ...
        if "idToken" in r:
            return {"ok":True,"auth":r["idToken"],
                    "refresh_token":r.get("refreshToken",""),
                    "firebase_uid":r.get("localId","")}
        err = str(r.get("error",{}).get("message","")).upper()
        for k in ["EMAIL_NOT_FOUND","INVALID_PASSWORD","INVALID_LOGIN_CREDENTIALS",
                  "TOO_MANY_ATTEMPTS","USER_DISABLED","INVALID_EMAIL"]:
    ...
    async def register(self, email, password):
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout,
                ...
                async with s.post(url, json=p, headers=h) as resp:
                    text = await resp.text()
                    log.info(f"Register [{resp.status}] {email}: {text[:200]}")
                    try: r = json.loads(text)
                    except: return {"ok":False,"message":"NETWORK_ERROR"}
        except Exception as e:
        ...
        if "idToken" in r:
            return {"ok":True,"auth":r["idToken"],
                    ...
                    "firebase_uid":r.get("localId","")}
        err = str(r.get("error",{}).get("message","")).upper()
        return {"ok":False,"message":err[:80] or "UNKNOWN"}
    ...
    async def _refresh(self, uid):
        ...
        if em and pw:
            ...
            if res.get("ok"):
                self.save_token(uid,res["auth"],em,pw,
    ...
    async def load(self, uid, force=False):
        ...
        try:
            ...
            dec = decrypt_player_record(r["result"],td.get("firebase_uid",""),
    ...
    async def _send_fresh(self, auth, record, fuid, original=None):
        """
        Отправка с полной диагностикой. Возвращает (ok, msg, raw_response).
        Используется для отладки, но НЕ используется в _save_two_step.
        """
        if not fuid: return False,"NO_UID",None
        try:
            payload = build_payload(record, fuid, original)
            log.info(f"  [_send_fresh] keys={sorted(record.keys())}, payload={len(payload)/1024:.1f}KB")
            r = await self._post(SAVE_URL,
                {"data":{"data":payload,"deviceId":fuid[:8]}},
                {**GAME_HEADERS,"Authorization":f"Bearer {auth}","Connection":"Keep-Alive",
                 "User-Agent":"Dalvik/2.1.0 (Linux; U; Android 12; Pixel 6 Build/SD1A.210817.036)"})
            ok = bool(r and self._ok(r))
            msg = "OK" if ok else f"SAVE_FAILED: {str(r)[:200]}"
            return ok, msg, r
        except Exception as e:
    ...
    async def set_rank(self, uid):
        ...
        r = await self._post(RANK_URL,{"data":json.dumps(rd)},
                             {**GAME_HEADERS,"Authorization":f"Bearer {auth}"})
        if r and self._ok(r):
    ...
    async def _auto_load_direct(self, auth: str, fuid: str, email: str) -> dict:
        """Загружает запись напрямую по auth+fuid (без БД)."""
        try:
            r = await self._post(LOAD_URL, {"data": None},
                                 {**GAME_HEADERS, "Authorization": f"Bearer {auth}"})
            if not r or not r.get("result"): return {}
            dec = decrypt_player_record(r["result"], fuid, None, email)
            return dec.get("record", {}) if dec.get("success") else {}
        except Exception as e:
            log.error(f"auto_load: {e}"); return {}

    async def _auto_save_fields(self, auth: str, fuid: str, email: str, mods: dict):
        rec = await self._auto_load_direct(auth, fuid, email)
        if not rec or not rec.get("Name"): return
        for k, v in mods.items(): rec[k] = v
        await self._send(auth, rec, fuid, original=None)

    async def _auto_patch_floats(self, auth, fuid, email, idx_val: dict):
        rec = await self._auto_load_direct(auth, fuid, email)
        if not rec or not rec.get("Name"): return
        fl = rec.get("floats", [])
        max_i = max(idx_val.keys())
        while len(fl) <= max_i: fl.append(0.0)
        for i, v in idx_val.items(): fl[i] = float(v)
        rec["floats"] = fl
        await self._send(auth, rec, fuid, original=None)

    async def _auto_patch_ints(self, auth, fuid, email, idx_val: dict):
        rec = await self._auto_load_direct(auth, fuid, email)
        if not rec or not rec.get("Name"): return
        it = rec.get("integers", [])
        max_i = max(idx_val.keys())
        while len(it) <= max_i: it.append(0)
        for i, v in idx_val.items(): it[i] = int(v)
        rec["integers"] = it
        await self._send(auth, rec, fuid, original=None)

    async def _auto_patch_animations(self, auth, fuid, email):
        rec = await self._auto_load_direct(auth, fuid, email)
        if not rec or not rec.get("Name"): return
        rec["animations"] = list(set(rec.get("animations", []) + list(range(301))))
        await self._send(auth, rec, fuid, original=None)

    async def _auto_patch_wheels(self, auth, fuid, email):
        rec = await self._auto_load_direct(auth, fuid, email)
        if not rec or not rec.get("Name"): return
        rec["wheels"] = list(set(rec.get("wheels", []) + list(range(73, 221))))
        it = rec.get("integers", [])
        while len(it) < 113: it.append(0)
        for i in [0,1,2,3,4,5,110,111,112]: it[i] = 1
        rec["integers"] = it
    ...
    async def auto_max_account(self, auth: str, fuid: str, email: str):
        """Автонакрутка: money/coins, ранг, W16, horns, smoke, fuel, no-dmg, anims, wheels, houses."""
        try:
            await self._auto_save_fields(auth, fuid, email, {
                "money": MAX_MONEY,
                "coin":  MAX_COIN,
            })
        except Exception as e:
            log.error(f"auto_money: {e}")
        ...
        try:
            rd = {"RatingData":{"time":1e22,"cars":1e16,"car_fix":1e13,"car_collided":1e12,
                ...
                "t_distance":1e10,"treasure":1e10,"block_post":1e10,"push_ups":1e12,
                "burnt_tire":1e10,"passanger_distance":1e8}}
            await self._post(RANK_URL, {"data": json.dumps(rd)},
                             {**GAME_HEADERS, "Authorization": f"Bearer {auth}"})
        except Exception as e:
            log.error(f"auto_rank: {e}")

        try:
            await self._auto_patch_floats(auth, fuid, email, {
                3: 1.0, 27: 1.0, 28: 1.0, 29: 1.0, 30: 1.0, 31: 1.0,
                32: 1.0, 33: 1.0, 34: 1.0,
            })
            await self._auto_patch_ints(auth, fuid, email, {
                8: 1, 110: 1, 111: 1, 112: 1,
            })
            await self._auto_patch_animations(auth, fuid, email)
            await self._auto_patch_wheels(auth, fuid, email)
        except Exception as e:
            log.error(f"auto_features: {e}")

    # ═══════════════════════════════════════════
    #  🚀 TWO-STEP SAVE
    # ═══════════════════════════════════════════

    async def _save_two_step(self, auth: str, record: dict, fuid: str, email: str):
        """
        Двухшаговое сохранение:
        1) Базовые поля + carIDnStatus (машины) + fcar + wheels + allData
        2) Пауза 3 сек, затем platesData + друзья + уровни + остальное
        """
        # ШАГ 1
        step1 = {k: v for k, v in record.items() if k in {
            "localID", "money", "Name", "coin", "allData",
            "carIDnStatus", "fcar", "wheels",
        }}
        log.info(f"═══ _save_two_step ═══")
        log.info(f"  STEP1 keys: {sorted(step1.keys())}")
        log.info(f"  cids count: {len((step1.get('carIDnStatus') or {}).get('carGeneratedIDs', []))}")
        ok1, msg1 = await self._send(auth, step1, fuid, original=None)
        log.info(f"  STEP1 result: ok={ok1}, msg={msg1}")
        if not ok1:
            return False, f"STEP1_FAIL: {msg1}"

        await asyncio.sleep(3)

        # ШАГ 2
        step2 = {k: v for k, v in record.items() if k in {
            "platesData", "FriendsID", "LevelsDoneTime", "floats",
            "integers", "boughtFsos", "boughtPoliceLights", "boughtPoliceSirens",
            "favouriteWheels", "favouriteVinyls", "favouriteEmojis",
        }}
        log.info(f"  STEP2 keys: {sorted(step2.keys())}")
        log.info(f"  plates count: {len((step2.get('platesData') or {}).get('allPlates', []))}")
        ok2, msg2 = await self._send(auth, step2, fuid, original=None)
        log.info(f"  STEP2 result: ok={ok2}, msg={msg2}")
        if not ok2:
            return False, f"STEP2_FAIL: {msg2}"

        log.info(f"═══ _save_two_step: SUCCESS ═══")
        return True, "OK"
...
class T:
    ...
    def login_fail(reason):
        err_map = {
            "EMAIL_NOT_FOUND":"Email not found","INVALID_PASSWORD":"Wrong password",
            "INVALID_LOGIN_CREDENTIALS":"Invalid credentials",
            "TOO_MANY_ATTEMPTS":"Too many attempts, wait",
            "USER_DISABLED":"Account disabled","INVALID_EMAIL":"Invalid email format",
...
class K:
    ...
    def home(uid=0):
        rows = [
            ...
            [InlineKeyboardButton(text="👥 Clone Accounts", callback_data="menu_clone"),
    ...
    def clone_menu():
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="👤 Clone Single Account", callback_data="clone_single")],
            [InlineKeyboardButton(text="👥 Bulk Clone Accounts",  callback_data="clone_bulk")],
            [InlineKeyboardButton(text="◂ Back", callback_data="back_home")],
        ])
    ...
    def admin(uid):
        ...
        b   = []
        if lvl >= 5:
            b.append([InlineKeyboardButton(text="📊 Stats", callback_data="a_stats"),
        ...
        if lvl >= 10:
            ...
            b.append([InlineKeyboardButton(text="➕ Add",   callback_data="a_adduser"),
            ...
            b.append([InlineKeyboardButton(text="👢 Kick",    callback_data="a_kick"),
                      InlineKeyboardButton(text="⏰ Expiry",  callback_data="a_expiry"),
                      InlineKeyboardButton(text="ℹ Profile", callback_data="a_profile")])
        if lvl >= 50:
            b.append([InlineKeyboardButton(text="💎 +VIP", callback_data="a_addvip"),
        ...
        if lvl >= 100:
            b.append([InlineKeyboardButton(text="➕ Add Admin", callback_data="a_addadm"),
                      InlineKeyboardButton(text="➖ Rem Admin", callback_data="a_rmadm")])
            b.append([InlineKeyboardButton(text="🖼 Update Photo", callback_data="a_photo"),
    ...
    def pending_list():
        ...
        for uid_str, info in list(PENDING.items())[:15]:
            ...
            b.append([InlineKeyboardButton(text=f"✅ {name}", callback_data=f"rq_accept_{uid_int}"),
...
def _gen_password(n: int = 14) -> str:
    ...
    pw  += [random.choice(pool) for _ in range(n - 3)]
    random.shuffle(pw)
    return "".join(pw)


def _gen_email() -> str:
    return f"{_rand_str(12)}@{CLONE_DOMAIN}"


def _gen_player_id(existing: str = "") -> str:
    m = re.match(r"^([A-Za-z]+)[\-\s]?(\d+)$", existing or "")
    if m:
        return f"{m.group(1)}{int(m.group(2)) + random.randint(100, 9999)}"
    return ("".join(random.choice(string.ascii_uppercase) for _ in range(2))
            + "".join(random.choice(string.digits) for _ in range(5)))


def _build_clone_record(source: dict) -> dict:
    """
    Клонирует: машины, деньги, коины, друзья, уровни, plates, wins/loses,
    полицейские фичи, колёса, FSOs, избранное.
    НЕ клонирует: localID, Name, скины персонажа, анимации, эмодзи, флаги.
    """
    clone = deepcopy(source)

    clone["localID"] = _gen_player_id(source.get("localID", ""))
    clone["Name"]    = f"{source.get('Name','Player')}_{_rand_str(4)}"
    ...
    clone = {k: v for k, v in clone.items() if k in KEEP}

    clone.setdefault("Name", f"Player_{_rand_str(4)}")
    clone.setdefault("localID", _gen_player_id(""))
    clone.setdefault("FriendsID", [])
    clone.setdefault("floats", [])
    clone.setdefault("integers", [])
    clone.setdefault("LevelsDoneTime", [])
    clone.setdefault("personEquipmentsMale", None)
    clone.setdefault("personEquipmentsFemale", None)
    clone.setdefault("platesData", None)
    clone.setdefault("carIDnStatus", None)
    clone.setdefault("flags", {})
    clone.setdefault("animations", [])
    clone.setdefault("emojiPacks", [])
    clone.setdefault("wheels", [])
    clone.setdefault("boughtFsos", [])
    clone.setdefault("boughtPoliceLights", [])
    clone.setdefault("boughtPoliceSirens", [])
    clone.setdefault("fcar", [])
    clone.setdefault("favouriteWheels", [])
    clone.setdefault("favouriteVinyls", [])
    clone.setdefault("favouriteEmojis", [])
    return clone


def _clone_limit(uid: int) -> int:
    return CLONE_MAX_OWNER if uid == OWNER_ID else CLONE_MAX_DEFAULT


def _clone_cooldown_left(uid: int) -> int:
    if uid == OWNER_ID: return 0
    return max(0, int(CLONE_COOLDOWN.get(uid, 0) + CLONE_COOLDOWN_SEC - time.time()))


def _clone_set_cooldown(uid: int):
    if uid != OWNER_ID:
        CLONE_COOLDOWN[uid] = time.time()
...
async def cb_clone_root(cb: CallbackQuery, state: FSMContext):
    await state.clear()
    uid = cb.from_user.id
    if not nuker.get_token(uid):
        await cb.answer("✗ Sign in first!", show_alert=True); return
    limit = _clone_limit(uid)
    left  = _clone_cooldown_left(uid)
    cd    = f"⏳ Cooldown: <b>{_fmt_secs(left)}</b>" if left else "✅ Ready"
    await cb.message.edit_text(
        f"{B}\n  👥  𝗖𝗟𝗢𝗡𝗘 𝗔𝗖𝗖𝗢𝗨𝗡𝗧𝗦\n{B}\n\n"
        f"  ▸ <b>Single</b> — clone progress into 1 account\n"
        f"  ▸ <b>Bulk</b> — register many & clone\n\n"
        f"  ◆ Your limit: <b>{limit}</b> per batch\n  ◆ {cd}",
        reply_markup=K.clone_menu())
    await cb.answer()


@rt.callback_query(F.data == "clone_single")
async def cb_clone_single(cb: CallbackQuery, state: FSMContext):
    uid = cb.from_user.id
    if not nuker.get_token(uid):
        await cb.answer("✗ Sign in first!", show_alert=True); return
    await cb.answer()
    m = await cb.message.edit_text("  ⏳ Loading donor account...")
    await nuker.load(uid, force=False)
    td    = nuker.get_token_data(uid)
    email = td.get("email") if td else None
    donor = nuker.get_record(uid, email)
    if not donor or not donor.get("Name"):
        await result_msg(m, False, "𝗖𝗟𝗢𝗡𝗘 𝗘𝗥𝗥𝗢𝗥",
            "Donor account not loaded.\nRefresh first.", K.clone_menu())
        return
    await state.set_state(SClone.single_email)
    await m.edit_text(
        f"{B}\n  👤  𝗦𝗜𝗡𝗚𝗟𝗘 𝗖𝗟𝗢𝗡𝗘\n{B}\n\n"
        f"  ╭──── 𝗗𝗢𝗡𝗢𝗥 ────╮\n"
        f"  │ 👤 {donor.get('Name','?')}\n  │ 🆔 {donor.get('localID','—')}\n"
        f"  │ 💰 ${donor.get('money',0):,}\n"
        f"  ╰────────────────╯\n\n"
        f"  ▸ Enter <b>target account email</b>:",
        reply_markup=K.cancel())


@rt.message(SClone.single_email)
async def p_clone_single_email(msg: Message, state: FSMContext):
    em = (msg.text or "").strip().lower()
    ...
    await state.set_state(SClone.single_password)
    await msg.answer(
        f"{B}\n  🔑  𝗧𝗔𝗥𝗚𝗘𝗧 𝗣𝗔𝗦𝗦𝗪𝗢𝗥𝗗\n{B}\n\n"
        f"  ▸ Target: <code>{em}</code>\n  ▸ Enter its password:\n  🔒 Deleted after use",
        reply_markup=K.cancel())


@rt.message(SClone.single_password)
async def p_clone_single_password(msg: Message, state: FSMContext):
    ...
    if not target or not pw:
        await msg.answer("  ✗ Missing credentials.", reply_markup=K.clone_menu()); return
    status = await msg.answer(f"  ⏳ Logging in as <code>{target}</code>...")
    lg = await nuker.login(target, pw)
    if not lg.get("ok"):
        await result_msg(status, False, "𝗧𝗔𝗥𝗚𝗘𝗧 𝗟𝗢𝗚𝗜𝗡 𝗙𝗔𝗜𝗟𝗘𝗗",
            lg.get("message","?"), K.clone_menu()); return
    td    = nuker.get_token_data(uid)
    dm    = td.get("email") if td else None
    donor = nuker.get_record(uid, dm)
    if not donor or not donor.get("Name"):
        await result_msg(status, False, "𝗗𝗢𝗡𝗢𝗥 𝗘𝗥𝗥𝗢𝗥",
            "Donor lost. Refresh.", K.clone_menu()); return
    clone = _build_clone_record(donor)
    try: await status.edit_text(f"  ⏳ Writing progress to <code>{target}</code>...")
    except: pass
    saved, smsg = await nuker._save_two_step(
        lg["auth"], clone, lg.get("firebase_uid",""), target)
    if saved:
        try:
            await status.edit_text(f"  ⚙️ Auto-max для <code>{target}</code>...")
            await nuker.auto_max_account(lg["auth"], lg.get("firebase_uid",""), target)
        except Exception as e:
            log.error(f"auto_max single: {e}")
        admin_log(uid, "CLONE_SINGLE_OK", target)
        await result_msg(status, True, "𝗦𝗜𝗡𝗚𝗟𝗘 𝗖𝗟𝗢𝗡𝗘 𝗗𝗢𝗡𝗘",
            f"  ✔ Target:    <code>{target}</code>\n"
            f"  ✔ New PID:   <code>{clone['localID']}</code>\n"
            f"  ✔ New Name:  <b>{clone.get('Name','?')}</b>\n"
            f"  ✔ Money:     ${clone.get('money',0):,}\n"
            f"  ✔ Auto-max:  ✓ applied",
            K.clone_menu())
    else:
        admin_log(uid, "CLONE_SINGLE_FAIL", target)
        await result_msg(status, False, "𝗦𝗔𝗩𝗘 𝗙𝗔𝗜𝗟𝗘𝗗",
            smsg or "?", K.clone_menu())


@rt.callback_query(F.data == "clone_bulk")
async def cb_clone_bulk(cb: CallbackQuery, state: FSMContext):
    uid = cb.from_user.id
    if not nuker.get_token(uid):
        await cb.answer("✗ Sign in first!", show_alert=True); return
    left = _clone_cooldown_left(uid)
    if left > 0:
        await cb.answer(f"⏳ Cooldown: {_fmt_secs(left)}", show_alert=True); return
    await cb.answer()
    m = await cb.message.edit_text("  ⏳ Loading donor account...")
    await nuker.load(uid, force=False)
    td    = nuker.get_token_data(uid)
    ...
    if not donor or not donor.get("Name"):
        await result_msg(m, False, "𝗖𝗟𝗢𝗡𝗘 𝗘𝗥𝗥𝗢𝗥",
            "Donor not loaded.\nRefresh first.", K.clone_menu()); return
    limit = _clone_limit(uid)
    await state.set_state(SClone.bulk_count)
    await m.edit_text(
        f"{B}\n  👥  𝗕𝗨𝗟𝗞 𝗖𝗟𝗢𝗡𝗘\n{B}\n\n"
        f"  ╭──── 𝗗𝗢𝗡𝗢𝗥 ────╮\n"
        f"  │ 👤 {donor.get('Name','?')}\n  │ 🆔 {donor.get('localID','—')}\n"
        f"  │ 💰 ${donor.get('money',0):,}\n"
        f"  ╰────────────────╯\n\n"
        f"  ◆ Domain:  <code>@{CLONE_DOMAIN}</code>\n"
        f"  ◆ Max:     <b>{limit}</b>\n"
        f"  ◆ Cooldown after: <b>{_fmt_secs(CLONE_COOLDOWN_SEC)}</b>\n\n"
        f"  ▸ Enter number of accounts to create:",
        reply_markup=K.cancel())


@rt.message(SClone.bulk_count)
async def p_clone_bulk_count(msg: Message, state: FSMContext):
    uid   = msg.from_user.id
    limit = _clone_limit(uid)
    left  = _clone_cooldown_left(uid)
    if left > 0:
        await state.clear()
        await msg.answer(f"  ⏳ Cooldown: {_fmt_secs(left)}", reply_markup=K.clone_menu()); return
    try:
        count = int((msg.text or "").strip())
        assert 1 <= count <= limit
    except:
        await msg.answer(f"  ✗ Enter 1 — {limit}:", reply_markup=K.cancel()); return
    await state.clear()
    td       = nuker.get_token_data(uid)
    src_mail = td.get("email") if td else None
    donor    = deepcopy(nuker.get_record(uid, src_mail))
    if not donor or not donor.get("Name"):
        await msg.answer("  ✗ Donor lost. Refresh.", reply_markup=K.clone_menu()); return
    ...
    for i in range(count):
        ...
        pw    = _gen_password(14)
        try:
            reg = await nuker.register(email, pw)
            if not reg.get("ok") and "TOO_MANY_ATTEMPTS" in reg.get("message",""):
                wait = 60 + random.randint(0, 30)
                results.append(f"[RATE-LIMIT] pause {wait}s for {email}")
                await asyncio.sleep(wait)
                reg = await nuker.register(email, pw)
            if not reg.get("ok"):
                results.append(f"[FAIL-REG] {email}:{pw} — {reg.get('message','?')}")
            else:
                clone = _build_clone_record(donor)
                saved, smsg = await nuker._save_two_step(
                    reg["auth"], clone, reg["firebase_uid"], email)
                if saved:
                    try:
                        await nuker.auto_max_account(
                            reg["auth"], reg["firebase_uid"], email)
                    except Exception as e:
                        log.error(f"auto_max {email}: {e}")
                    ok_cnt += 1
                    results.append(
                        f"[OK] {email}:{pw}\n"
                        f"     pid={clone['localID']}  "
                        f"name={clone.get('Name','?')}  "
                        f"uid={reg['firebase_uid']}")
                else:
                    results.append(f"[FAIL-SAVE] {email}:{pw} — {smsg}")
        except Exception as ex:
            results.append(f"[ERR] {email}:{pw} — {ex}")
        ...
        try:
            await status.edit_text(
                f"{B}\n  👥  𝗖𝗟𝗢𝗡𝗜𝗡𝗚\n{B}\n\n"
                f"  [{bar}] {pct}%\n"
                f"  ▸ {i+1}/{count}  ✔ {ok_cnt}  ✗ {len(results)-ok_cnt}\n\n"
                f"  ⏳ @{CLONE_DOMAIN}")
        except: pass
        await asyncio.sleep(CLONE_DELAY_SEC + random.uniform(0, 0.5))
    report  = "═══ BULK CLONE REPORT ═══\n"
    report += f"Date:      {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    report += f"Donor:     {donor.get('Name','?')} ({src_mail})\n"
    report += f"Domain:    @{CLONE_DOMAIN}\n"
    report += f"Requested: {count}\nSuccess:   {ok_cnt}\nFailed:    {count - ok_cnt}\n"
    report += "═════════════════════════\n\n" + "\n".join(results)
    file = BufferedInputFile(report.encode("utf-8"),
                             filename=f"clones_{uid}_{int(time.time())}.txt")
    admin_log(uid, "CLONE_BULK", f"ok={ok_cnt}/{count}")
    _clone_set_cooldown(uid)
    try: await status.delete()
    except: pass
    left_now = _clone_cooldown_left(uid)
    cd_line  = f"\n  ⏳ Next bulk in: <b>{_fmt_secs(left_now)}</b>" if left_now else ""
    await msg.answer_document(
        document=file,
        caption=(
            f"{B}\n  🎉  𝗕𝗨𝗟𝗞 𝗖𝗟𝗢𝗡𝗘 𝗗𝗢𝗡𝗘\n{B}\n\n"
            f"  ✔ Success: <b>{ok_cnt}/{count}</b>\n"
            f"  ✗ Failed:  <b>{count-ok_cnt}</b>\n"
            f"  🌐 Domain:  <code>@{CLONE_DOMAIN}</code>{cd_line}\n\n"
            f"  ▸ Credentials attached."),
        reply_markup=K.clone_menu())
# ═══════════════════════════════════════════
#  🚀 /start
# ═══════════════════════════════════════════
...
async def cb_login(cb: CallbackQuery, state: FSMContext):
    ...
    if not is_allowed(uid):
        await cb.message.edit_text(T.no_access(), reply_markup=K.no_access()); await cb.answer(); return
    if is_maintenance() and not has_admin(uid,"moderator"):
        await cb.message.edit_text(T.maintenance()); await cb.answer(); return
...
async def p_pass(msg: Message, state: FSMContext):
    ...
    try:
        ...
        if r.get("ok"):
            nuker.save_token(uid, r["auth"], em, pw,
                             r.get("refresh_token",""), r.get("firebase_uid",""))
...
async def cb_coins(cb: CallbackQuery, state: FSMContext):
    ...
    await result_msg(m, r.get("ok"),
        ...
        f"🪙 {fmt(int(v))} coins" if r.get("ok") else "",
        K.back_home())
...
async def cb_feat(cb: CallbackQuery):
    ...
    await result_msg(m, r.get("ok"),
        ...
        "" if r.get("ok") else r.get("message",""),
        K.feat())
...
async def p_name(msg: Message, state: FSMContext):
    ...
    await result_msg(ld, r.get("ok"),
        "𝗡𝗔𝗠𝗘 𝗨𝗣𝗗𝗔𝗧𝗘𝗗" if r.get("ok") else "𝗙𝗔𝗜𝗟𝗘𝗗",
...
async def p_pid(msg: Message, state: FSMContext):
    ...
    await result_msg(ld, r.get("ok"),
        "𝗜𝗗 𝗨𝗣𝗗𝗔𝗧𝗘𝗗" if r.get("ok") else "𝗙𝗔𝗜𝗟𝗘𝗗",
...
async def p_wins(msg: Message, state: FSMContext):
    ...
    await result_msg(ld, r.get("ok"),
        "𝗪𝗜𝗡𝗦 𝗨𝗣𝗗𝗔𝗧𝗘𝗗" if r.get("ok") else "𝗙𝗔𝗜𝗟𝗘𝗗",
...
async def p_loses(msg: Message, state: FSMContext):
    ...
    await result_msg(ld, r.get("ok"),
        "𝗟𝗢𝗦𝗘𝗦 𝗨𝗣𝗗𝗔𝗧𝗘𝗗" if r.get("ok") else "𝗙𝗔𝗜𝗟𝗘𝗗",
...
async def cmd_admin(msg: Message, state: FSMContext):
    ...
    if not has_admin(uid,"moderator"):
        await msg.answer("  ✗ No admin access."); return
        ...
        if await state.get_state() == SAdmin.bulkadd.state:
            ...
            if data.get("bulkadd_user") == user_id:
                ...
                await bot.send_message(chat_id,
                    "  ⌛ Bulk add cancelled: timed out waiting for user IDs.",
...
async def cb_admin_menu(cb: CallbackQuery, state: FSMContext):
    ...
    uid = cb.from_user.id
    if not has_admin(uid,"moderator"):
        await cb.answer("✗ No access!", show_alert=True); return
...
async def p_bulkadd(msg: Message, state: FSMContext):
    ...
    if not raw_lines:
        await msg.answer("  ✗ Empty submission. Send one user ID per line, or /cancel to cancel.",
                         reply_markup=K.back_admin())
        return
...
async def p_ban(msg: Message, state: FSMContext):
    ...
    if uid == OWNER_ID:
        await msg.answer("  ✗ Cannot ban owner!", reply_markup=K.back_admin())
...
async def p_kick(msg: Message, state: FSMContext):
    ...
    if uid == OWNER_ID:
        await msg.answer("  ✗ Cannot kick owner!", reply_markup=K.back_admin())
        await state.clear(); return
    await state.clear()
...
async def p_expiry_id(msg: Message, state: FSMContext):
    ...
    await msg.answer(f"  Enter days for <code>{uid}</code> (0 = remove):",
                     reply_markup=K.back_admin())
...
async def p_expiry_dy(msg: Message, state: FSMContext):
    ...
    if days == 0:
        ...
        await msg.answer(f"  ✔ Expiry removed for <code>{uid}</code>",
                         reply_markup=K.back_admin())
    else:
        ...
        await msg.answer(f"  ✔ <code>{uid}</code> expires in {days} days",
                         reply_markup=K.back_admin())
...
async def cb_bcast_text(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        hdr("📢","𝗧𝗘𝗫𝗧 𝗕𝗥𝗢𝗔𝗗𝗖𝗔𝗦𝗧")+f"\n\n  Message to all {len(ALLOWED_USERS)} users:",
        reply_markup=K.back_admin())
    await cb.answer()
...
async def cb_sl(cb: CallbackQuery, state: FSMContext):
    ...
    try: await cb.message.edit_text(f"  ✔ <code>{t}</code> → {role}",
                                    reply_markup=K.back_admin())
    except: pass
...
async def p_rmadm(msg: Message, state: FSMContext):
    ...
    if uid == OWNER_ID:
        await msg.answer("  ✗ Cannot remove owner!", reply_markup=K.back_admin())
        await state.clear(); return
    await state.clear()
